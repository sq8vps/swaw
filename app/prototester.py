...
def ParseSpo2(payload, size):
    ...
    for i in range(SPO2_NUM_OF_SAMPLE-1):
        ir_recv[i] = payload[i*SPO2_SAMPLE_SIZE] << 16 | payload[(i*SPO2_SAMPLE_SIZE)+1] << 8 | payload[(i*SPO2_SAMPLE_SIZE)+1]
    for i in range(SPO2_NUM_OF_SAMPLE-1):
        red_recv[i] = payload[(i*SPO2_SAMPLE_SIZE)+SPO2_NUM_OF_SAMPLE] << 16 | \
                      payload[(i*SPO2_SAMPLE_SIZE)+1+SPO2_NUM_OF_SAMPLE] << 8 | \
                      payload[(i*SPO2_SAMPLE_SIZE)+1+SPO2_NUM_OF_SAMPLE]
